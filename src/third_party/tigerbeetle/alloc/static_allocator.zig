...
// Vendored from tigerbeetle/src/static_allocator.zig @ 44544ee11057bbc8fe826cb7f93e8e00a57f2fc1.
// Modifications: none (self-contained; only depends on std).
...
// ── Tests (added in speedy-socials, not part of upstream TB) ─────────

const testing = std.testing;

test "StaticAllocator: init phase allows alloc/free" {
...
test "StaticAllocator: deinit transition re-allows free" {
    var sa = StaticAllocator.init(testing.allocator);
    defer sa.deinit();
    const a = sa.allocator();
