// Vendored from tigerbeetle/src/stdx/prng.zig @ 44544ee11057bbc8fe826cb7f93e8e00a57f2fc1.
// Modifications: `stdx` import redirected to local `_shim.zig` (a minimal
// reimplementation of Snap / cut / BitSetType / KiB / Flags used only by
// this file's tests — production code paths are unaffected).
...
const stdx = @import("_shim.zig");
const assert = std.debug.assert;
const math = std.math;
const Snap = stdx.Snap;
const module_path = "src/third_party/tigerbeetle/prng";
...
test "no floating point please" {
    // PATCH: read via a hard-coded path relative to project root rather
    // than `@src().file` (whose exact value varies with build layout).
    const path = "src/third_party/tigerbeetle/prng/prng.zig";

    const file_text = std.fs.cwd().readFileAlloc(std.testing.allocator, path, 64 * KiB) catch |err| switch (err) {
        // Build harness may run tests from a different cwd; skip rather
