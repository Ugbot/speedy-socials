...
pub fn build(b: *std.Build) void {
    ...
    // Per-plugin test binaries. Each plugin's `plugin.zig` references
    // its submodules in a top-level `test {}` block, so testing the
    // plugin root file picks up everything in the module.
    for (plugin_modules) |pm| {
        ...
        const plugin_test = b.addTest(.{ .root_module = test_mod });
        const run_plugin_test = b.addRunArtifact(plugin_test);
        test_step.dependOn(&run_plugin_test.step);
    }
}
